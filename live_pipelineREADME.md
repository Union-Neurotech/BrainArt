...
variant over WebSocket. This file is the index for the live pipeline; see
`window_experiment_findings.md` for the empirical justification behind the design.
...
| File | Role |
|---|---|
| `train_muse_va_live.py` | trains the 30 s engineered-feature Ridge model → `muse_va_live.npz` (+`.json`) |
| `muse_va_live.npz` / `.json` | trained model (numpy-only) + metadata |
| `brainart_muse_server.py` | Muse → baseline → sliding-window inference → calibration → WebSocket |
| `faced_window_experiment.py` | the experiment that produced the findings |
| `window_experiment_findings.md` | full results table + recommendation |
| web client | `…/Code/BrainArt/new_web_variant/brainart_emotion.js` (has **go live** + **Calibration** UI) |

## Run
```bash
pip install brainflow websockets numpy scikit-learn pandas
python train_muse_va_live.py                 # optional: retrain (CV r_val≈.15 r_aro≈.12)
...
```
Open `new_web_variant/brainart_emotion.html` → **go live** (top-right). Status
