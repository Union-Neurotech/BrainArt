...
// --- Vertex shader (glsl) --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------â”€
import { VERTEX_SHADER as VS } from './vertex.js';

// ------- Fragment shader (glsl) --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------â”€
// This is a tiny randomly-weighted neural network (specifically a 
// Compositional Pattern Producing Network) evaluated per-pixel.
// W(layer, input, output) and Wb(layer, output) produce deterministic weights
// from the seed value, so every seed gives a different painting.
...
function mkSlider(parent, { key, label, hint, min, max, step, cls }) {
  ...
  const fmt = n => step < 0.1 ? n.toFixed(2) : n.toFixed(1);
  row.innerHTML = `
    <div class="sl-label">${label}<small>${hint}</small></div>
    <input type="range" class="${cls}" id="sl_${key}" min="${min}" max="${max}" step="${step}" value="${E[key]}">
    <div class="sl-val" id="sv_${key}">${fmt(E[key])}</div>
  `;
  parent.appendChild(row);
  row.querySelector('input').addEventListener('input', function () {
    E[key] = +this.value;
    document.getElementById('sv_' + key).textContent = fmt(+this.value);
  });
}
...
function applyPreset(p) {
  ...
  Object.keys(p).forEach(k => {
    ...
    val.textContent = step < 0.1 ? E[k].toFixed(2) : E[k].toFixed(1);
  });
}
